...
Polymer({
	...
	_handleTrackMove: function(e) {
		...
		const calcedLevel = Math.floor(fromEdge / this._currentDragContext.stepWidth);
		const normalizedCalcedLevel = getComputedStyle(this).direction === 'rtl'
			? (this._currentDragContext.maxLevelIndex) - calcedLevel
			: calcedLevel;
		const nextSelected = Math.max(0, Math.min(normalizedCalcedLevel, this._currentDragContext.maxLevelIndex));
